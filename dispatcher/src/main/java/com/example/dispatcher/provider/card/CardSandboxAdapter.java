package com.example.dispatcher.provider.card;
...
@Component
public class CardSandboxAdapter implements PayoutProvider {
    ...
    public Mono<ProviderResult> disburse(ProviderRequest req) {
        return Mono.delay(Duration.ofMillis(150))
                .map(t -> ProviderResult.success("CARD-" + UUID.randomUUID()))
                .onErrorResume(ex -> Mono.just(ProviderResult.failed("CARD_ERR", ex.getMessage())));
    }
}
